...
public sealed class NotificationEngineTests
{
    ...
    public void BuildNotifications_FiltersByLeadTimeWindow()
    {
        ...
        {
            new() { Id = "1", Title = "In Window", StartUtc = now.AddMinutes(2), EndUtc = now.AddMinutes(3), Provider = "P", CalendarId = "C" },
            new() { Id = "2", Title = "Out of Window", StartUtc = now.AddMinutes(10), EndUtc = now.AddMinutes(11), Provider = "P", CalendarId = "C" }
        };

        var sut = new NotificationEngine();
        var notifications = sut.BuildNotifications(events, now, leadTimeMinutes: 3);
    ...
    public void BuildNotifications_FiltersDismissedTitles()
    {
        ...
        var events = new List<CalendarEvent>
        {
            ...
            new() { Id = "2", Title = "Product Review", StartUtc = now.AddMinutes(2), EndUtc = now.AddMinutes(3), Provider = "P", CalendarId = "C" }
        };
    ...
    private sealed class FakeDismissedTitleRepository : IDismissedEventTitleRepository
    {
        ...
        {
            _items = titles
                .Select(x => new DismissedEventTitle { Title = x, DismissedAtUtc = DateTimeOffset.UtcNow })
                .ToList();
        }
        ...
        {
            var exists = _items.Any(x => string.Equals(x.Title, title, StringComparison.OrdinalIgnoreCase));
    ...
    public void BuildNotifications_RespectsSelectedCalendars()
    {
        ...
        var events = new List<CalendarEvent>
        {
            new() { Id = "1", Title = "Allowed", StartUtc = now.AddMinutes(2), EndUtc = now.AddMinutes(3), Provider = "Outlook365", CalendarId = "primary" },
            new() { Id = "2", Title = "Blocked", StartUtc = now.AddMinutes(2), EndUtc = now.AddMinutes(3), Provider = "Outlook365", CalendarId = "other" }
        };
    ...
    private sealed class FakeCalendarSelectionRepository : ICalendarSelectionRepository
    {
        ...
        public Task<IReadOnlyList<SelectedCalendar>> GetSelectedAsync(CancellationToken cancellationToken = default)
        {
            return Task.FromResult(_selected);
        }

        public Task SaveSelectedAsync(IReadOnlyList<SelectedCalendar> selectedCalendars, CancellationToken cancellationToken = default)
        {
            return Task.CompletedTask;
        }
    }
