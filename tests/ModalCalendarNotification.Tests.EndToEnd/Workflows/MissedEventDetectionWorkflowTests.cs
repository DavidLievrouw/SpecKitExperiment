...
public sealed class MissedEventDetectionWorkflowTests
{
    ...
    public async Task MissedEventDetectionWorkflow_DetectsEventsSinceLastRun()
    {
        ...
        {
            new() { Id = "1", Title = "Missed", StartUtc = now.AddHours(-2), EndUtc = now.AddHours(-1), Provider = "Outlook365", CalendarId = "primary" },
            new() { Id = "2", Title = "Future", StartUtc = now.AddHours(1), EndUtc = now.AddHours(2), Provider = "Outlook365", CalendarId = "primary" }
        };

        var missed = await service.RecoverMissedEventsAsync(events, now);

        missed.Count.ShouldBe(1);
        missed[0].Title.ShouldBe("Missed");
        (await repository.GetLastRunUtcAsync()).ShouldBe(now);
    }
    ...
    private sealed class InMemoryApplicationStateRepository : IApplicationStateRepository
    {
        ...
        public Task<DateTimeOffset?> GetLastRunUtcAsync(CancellationToken cancellationToken = default)
        {
            return Task.FromResult(_value);
        }

        public Task SetLastRunUtcAsync(DateTimeOffset timestampUtc, CancellationToken cancellationToken = default)
        {
            _value = timestampUtc;
